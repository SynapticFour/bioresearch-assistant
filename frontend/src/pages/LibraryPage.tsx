...
import type { Paper, RAGResponse } from "@/types";
...
export function LibraryPage() {
  ...
  const [libraryTab, setLibraryTab] = useState<"search" | "rag">("search");
  const [semanticQuery, setSemanticQuery] = useState("");
  const [semanticThreshold, setSemanticThreshold] = useState<number>(1.0);
  const [ragQuestion, setRagQuestion] = useState("");
  const [ragTopK, setRagTopK] = useState(5);
  const [ragLanguage, setRagLanguage] = useState<"de" | "en">("de");
  ...
  const ragMutation = useMutation({
    mutationFn: () =>
      libraryApi.rag(ragQuestion.trim(), ragTopK, ragLanguage),
    onSuccess: (data) => setRagResult(data),
    onError: (err: Error) => showError(err?.message ?? "RAG-Anfrage fehlgeschlagen."),
  });
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Tabs: Suche | Frag deine Bibliothek */}
      <div className="flex gap-2 border-b border-slate-200">
        ...
        >
          <Search className="inline-block h-4 w-4 mr-2" aria-hidden />
          Suche
        </button>
        <button
          type="button"
          onClick={() => setLibraryTab("rag")}
          className={`rounded-t-lg px-4 py-2 text-sm font-medium transition-colors ${
            libraryTab === "rag"
              ? "border border-b-0 border-slate-200 bg-white text-slate-800"
              : "text-slate-600 hover:bg-slate-100"
        ...
        >
          <Brain className="inline-block h-4 w-4 mr-2" aria-hidden />
      ...
      {libraryTab === "rag" && (
        <div className="space-y-4 rounded-lg border border-slate-200 bg-white p-4">
          <h2 className="text-lg font-semibold text-slate-800">
            Stelle eine Frage an deine Bibliothek
          </h2>
          <textarea
            value={ragQuestion}
            onChange={(e) => setRagQuestion(e.target.value)}
            placeholder="z.B.: Welche Nebenwirkungen von Metformin werden in meinen Papers erwähnt?"
            className="w-full rounded-lg border border-slate-300 px-3 py-2 text-slate-800 placeholder:text-slate-400 focus:border-primary focus:outline-none focus:ring-2 focus:ring-primary/20"
            rows={3}
            aria-label="Frage an die Bibliothek"
          />
          <div className="flex flex-wrap items-center gap-4">
            ...
            <div>
              ...
              <select
                value={ragLanguage}
                onChange={(e) => setRagLanguage(e.target.value as "de" | "en")}
                className="rounded border border-slate-300 px-3 py-2 text-sm"
              >
          ...
          {papers.length === 0 && (
            <div className="rounded-lg border border-amber-200 bg-amber-50 p-4 text-sm text-amber-900">
              Noch keine Papers in der Bibliothek. Bitte zuerst{" "}
              <button
                type="button"
                onClick={() => setLibraryTab("search")}
          ...
          {ragResult && (
            <div className="space-y-4 border-t border-slate-200 pt-4">
              <h3 className="font-semibold text-slate-800">💬 Antwort</h3>
              <div
              ...
              <div>
                <h4 className="mb-2 text-sm font-medium text-slate-700">
                  📚 Verwendete Quellen ({ragResult.sources.length} Papers)
                </h4>
                <ul className="space-y-2">
                  {ragResult.sources.map((src) => (
                    <li
                      key={src.pmid}
                      className="flex flex-wrap items-center justify-between gap-2 rounded border border-slate-100 bg-slate-50 p-2 text-sm"
                    >
                      <span
                        className={`inline-flex items-center gap-2 ${
                          src.similarity_score >= 80
                            ? "text-green-700"
                            : src.similarity_score >= 60
                              ? "text-yellow-700"
                              : "text-orange-700"
                        }`}
                      >
                        <span className="font-medium">
                          {Math.round(src.similarity_score)}%
                        </span>
                        {src.title.length > 60
              ...
              <p className="text-xs text-slate-500">Modell: {ragResult.model_used}</p>
              <p className="rounded border border-amber-200 bg-amber-50 p-3 text-sm text-amber-900">
                ⚠️ Antwort basiert nur auf deinen gespeicherten Papers (
                {ragResult.context_papers} von {papers.length} verwendet). Kein
                Ersatz für vollständige Literaturrecherche.
              </p>
            </div>
          )}
        </div>
