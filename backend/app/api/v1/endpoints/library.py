...
async def summarize_paper(
    ...
    if not abstract:
        # Fallback: Titel als Basis verwenden
        abstract = paper.title or ""
        if not abstract:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail=(
                    "Paper hat keinen Abstract und keinen Titel — Zusammenfassung nicht möglich."
                ),
            )
        abstract = f"[Kein Abstract verfügbar. Basierend auf dem Titel:]\n{abstract}"
    try:
        llm = LLMService()
...
@router.post(
    ...
    status_code=status.HTTP_200_OK,
    summary="Frag deine Bibliothek (RAG)",
    description="Natürlichsprachige Frage an gespeicherte Papers — LLM-Antwort mit Quellen.",
)
...
async def rag_ask(
    ...
    """RAG: Frage embedden, Top-K Papers holen, Kontext bauen, LLM antworten."""
    question = (body.question or "").strip()
    if not question:
        raise HTTPException(
            status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
            detail="question darf nicht leer sein",
        )
    service = RAGService()
    try:
        ...
        logger.info(
            ...
            extra={
                ...
                "papers_used": result.context_papers,
            },
        )
        return result
    except ValueError as e:
        if "Keine Papers" in str(e):
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail=(
