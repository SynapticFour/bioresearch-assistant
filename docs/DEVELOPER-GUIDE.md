...
- **Backend:** Python 3.11+, FastAPI, PostgreSQL mit pgvector. Abhängigkeiten per `pip install --require-hashes --no-deps --extra-index-url https://download.pytorch.org/whl/cpu -r backend/requirements.lock`. Alembic für Migrationen. Empfohlen: Gunicorn/Uvicorn hinter Reverse-Proxy (nginx), HTTPS.
